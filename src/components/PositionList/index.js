...
const DashGrid = styled.div`
  ...
  grid-template-areas: 'number name saltswap return';
  ...
  @media screen and (max-width: 740px) {
    ...
    grid-template-areas: 'name saltswap return';
  }

  @media screen and (max-width: 500px) {
    grid-template-columns: 2.5fr 1fr;
    grid-template-areas: 'name saltswap';
  }
`
...
const SORT_FIELD = {
  VALUE: 'VALUE',
  saltswap_RETURN: 'saltswap_RETURN',
}
  ...
  const positionsSorted =
    ...
    positions
      ...
      .sort((p0, p1) => {
        ...
        if (sortedColumn === SORT_FIELD.saltswap_RETURN) {
          return p0?.saltswap?.return > p1?.saltswap?.return ? (sortDirection ? -1 : 1) : sortDirection ? 1 : -1
        }
        ...
        {!below500 && (
          <Flex alignItems="center" justifyContent="flexEnd">
            <ClickableText
              area="return"
              onClick={() => {
                setSortedColumn(SORT_FIELD.saltswap_RETURN)
                setSortDirection(sortedColumn !== SORT_FIELD.saltswap_RETURN ? true : !sortDirection)
              }}
            >
              {below740 ? 'Fees' : 'Total Fees Earned'}{' '}
              {sortedColumn === SORT_FIELD.saltswap_RETURN ? (!sortDirection ? '↑' : '↓') : ''}
            </ClickableText>
          </Flex>
        )}
      </DashGrid>
